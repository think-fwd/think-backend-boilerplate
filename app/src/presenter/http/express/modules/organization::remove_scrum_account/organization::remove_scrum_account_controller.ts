import { Request, Response } from 'express';
import { handleError } from '../../utils/handle_error';
import { IEventController } from '../../interfaces/event_controller_interface';
import { OrganizationRemoveScrumAccountUsecase } from '@domain/modules/organization::remove_scrum_account/organization::remove_scrum_account_usecase';

export class OrganizationRemoveScrumAccountController
  implements IEventController
{
  constructor(
    private readonly organizationRemoveScrumAccountUsecase: OrganizationRemoveScrumAccountUsecase,
  ) {}

  async handle(req: Request, res: Response) {
    try {
      const response = await this.organizationRemoveScrumAccountUsecase.handle({
        organizationId: req.params.id as unknown as string,
      });
      await res.status(200);
      return res.json(response);
    } catch (error: unknown) {
      return handleError(res, error as Record<string, unknown>);
    }
  }
}
